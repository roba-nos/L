...
/**
 * GET /api/proxy
 * Advanced Smart Proxy for HLS feeds: Rewrites manifests to bypass CORS
 */
app.get('/api/proxy', async (req, res) => {
    ...
    try {
        ...
        // Calculate Referer for spoofing
        let referer = '';
        try {
        ...
        // CRITICAL: Use the FINAL URL after all redirects for the base
        const finalUrl = response.url;
        const contentType = response.headers.get('content-type') || '';
        const isManifest = finalUrl.includes('.m3u8') || contentType.includes('mpegurl') || contentType.includes('application/x-mpegURL');
        ...
        if (response.status >= 400) {
            logStream(`🚩 Proxy Error [${response.status}] for: ${targetUrl.substring(0, 50)}...`);
            return res.status(response.status).send(`Target returned error: ${response.status}`);
        }

        if (isManifest) {
            const text = await response.text();
            // Use finalUrl to resolve relative segments correctly
            ...
            const rewritten = text.split('\n').map(line => {
                ...
                // Handle Tags with URIs (e.g. #EXT-X-KEY:URI="...", #EXT-X-MEDIA:URI="...")
                if (line.startsWith('#')) {
                    return line.replace(/URI="([^"]+)"/g, (match, uri) => {
                        const absoluteUri = uri.startsWith('http') ? uri : new URL(uri, baseUrl).href;
                        return `URI="${req.protocol}://${req.get('host')}/api/proxy?url=${encodeURIComponent(absoluteUri)}"`;
                    });
                }

                // Handle Segment URLs (Lines not starting with #)
                const absoluteSegmentUri = line.startsWith('http') ? line : new URL(line, baseUrl).href;
                return `${req.protocol}://${req.get('host')}/api/proxy?url=${encodeURIComponent(absoluteSegmentUri)}`;
            }).join('\n');

            res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
            return res.send(rewritten);
        } else {
            // Forward non-manifest resources directly
