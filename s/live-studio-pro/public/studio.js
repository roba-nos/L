...
const state = {
    ...
    engineUrl: localStorage.getItem('studio_engine_url') || ''
};

/**
 * Get API Endpoint based on connected engine
 * IMPORTANT: All /api/* calls MUST use this function to get an absolute URL
 */
function getEndpoint(path) {
    let base = (state.engineUrl || '').trim();
    
    if (!base) {
        // No engine URL set — warn user if it's an API call
        if (path.startsWith('/api/')) {
            console.warn('⚠️ Engine URL not set! API calls will fail on GitHub Pages. Set the engine URL in Settings.');
            addLocalLog('⚠️ لم يتم تعيين رابط السيرفر! يرجى تعيينه في الإعدادات.');
        }
        return path; // Fallback to relative (works only if served from same server)
    }
...
function playMedia(videoEl, url, hlsKey) {
    ...
    // Force Proxy via backend engine for cross-origin HLS
    // CRITICAL: Must be ABSOLUTE URL pointing to the engine server, not a relative path
    if (format === 'm3u8') {
        finalUrl = getEndpoint(`/api/proxy?url=${encodeURIComponent(url)}`);
    }
    ...
    if (format === 'm3u8') {
        if (window.Hls && Hls.isSupported()) {
            ...
            hls.on(window.Hls.Events.ERROR, async (event, data) => {
                if (data.fatal) {
                    const errorCode = data.response ? data.response.code : 'N/A';
                    let errorMsg = data.type;

                    // If it's a server-side 500, try to show the JSON message we added
                    if (errorCode === 500 && data.response && data.response.text) {
                        try {
                            const errJson = JSON.parse(data.response.text);
                            errorMsg = `Server Fault: ${errJson.message}`;
                        } catch(e) {}
                    }
                    
                    console.warn(`Hls Fatal Error [${hlsKey}]:`, data.type, `Status: ${errorCode}`);
                    addLocalLog(`❌ خطأ في المشغل (${errorCode}): ${errorMsg}`);

                    if (data.type === window.Hls.ErrorTypes.NETWORK_ERROR) {
                        // Diagnostic: Check for Mixed Content block
                        if (window.location.protocol === 'https:' && (state.engineUrl && (state.engineUrl.startsWith('http:') || finalUrl.startsWith('http:')))) {
                            console.error("⛔ Mixed Content Block: HTTPS Dashboard (GitHub) cannot call HTTP Engine (IP). Browser security block.");
                            alert("⚠️ خطأ أمني حرج: المتصفح يمنع الاتصال لأنك في بيئة HTTPS (GitHub). يرجى فتح الاستوديو عبر الرابط المباشر للسيرفر (HTTP).");
                        }
                        hls.startLoad();
                    }
    ...
    if (pushBtn) {
        pushBtn.onclick = async () => {
            ...
            try {
                ...
                const res = await fetch(getEndpoint('/api/stream/push-updates'), {
                    method: 'POST',
...
async function startStream() {
    ...
    try {
        ...
        const res = await fetch(getEndpoint('/api/stream/start'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function stopStream() {
    try {
        await fetch(getEndpoint('/api/stream/stop'), { method: 'POST' });
        setLiveState(false);
        stopStatusPolling();
        stopTimer();
    } catch (err) {
...
async function skipItem() {
    await fetch(getEndpoint('/api/stream/skip'), { method: 'POST' });
}
...
/**
 * Add a local log entry to the UI list
 */
...
async function fetchStatus() {
    ...
    try {
        const endpoint = getEndpoint('/api/stream/status');
        
        // Security Check: If on HTTPS (GitHub) and calling HTTP Engine
        ...
        if (!res.ok) {
            if (led) { led.classList.remove('status-led-online'); led.classList.add('status-led-offline'); }
            return; 
        }
        ...
        const data = await res.json();
        if (led) { led.classList.remove('status-led-offline'); led.classList.add('status-led-online'); }
        ...
        // Fetch Health Data
        const healthRes = await fetch(getEndpoint('/api/system/health'));
        if (healthRes.ok) {
...
function initStudio() {
    ...
    (async () => {
        ...
        // Check if engine URL input exists and handle it
        const engineInp = document.getElementById('engine-url-input');
        if (engineInp) {
            engineInp.value = state.engineUrl;
            engineInp.onchange = (e) => {
                state.engineUrl = e.target.value.trim();
                localStorage.setItem('studio_engine_url', state.engineUrl);
                console.log("🔗 Engine URL updated:", state.engineUrl);
                fetchStatus(); // Trigger immediate check
            };
        }

        // Check if stream is already running
        fetchStatus().then(() => {
            fetch(getEndpoint('/api/stream/status'))
                .then(r => r.json())
                .then(data => {
                    if (data.isLive) {
                        setLiveState(true);
...
// Initialize based on readyStore
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        initStudio();
        checkEmergencySecurity();
    });
} else {
    initStudio();
    checkEmergencySecurity();
...
function checkEmergencySecurity() {
    ...
    if (window.location.protocol === 'https:') {
        ...
        if (btn) {
            const currentEngine = state.engineUrl || '';
            const targetUrl = currentEngine ? `${currentEngine}/studio.html` : 'http://localhost:3000/studio.html';
            btn.href = targetUrl;
            btn.onclick = (e) => {
                if (!targetUrl.startsWith('http:')) {
                    alert("يرجى إدخال عنوان السيرفر أولاً في خانة الإعدادات.");
                    e.preventDefault();
                }
            };
        }
