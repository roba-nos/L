...
function getEndpoint(path) {
    if (!state.engineUrl) return path;
            ...
            hls.on(Hls.Events.ERROR, (event, data) => {
                if (data.fatal) {
                    ...
                    if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
                        ...
                        if (window.location.protocol === 'https:' && (state.engineUrl.startsWith('http:') || finalUrl.startsWith('http:'))) {
                            console.error("⛔ Mixed Content Block: HTTPS Dashboard (GitHub) cannot call HTTP Engine (IP). Browser security block.");
                            alert("⚠️ خطأ أمني: أنت تستخدم نسخة HTTPS (GitHub Pages) ولا يمكنها الاتصال بسيرفر محلي غير آمن (HTTP). يرجى تشغيل المشروع محلياً عبر http://localhost.");
                        }
                        hls.startLoad();
                    }
